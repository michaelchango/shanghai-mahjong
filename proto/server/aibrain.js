...
const RULES_BASE = [
  ...
  '清混碰锁门（view.lock）：view.lock 是锁定的花色字，取值「萬 / 筒 / 條」之一，null 表示尚未锁门。只要它非 null，你本局就锁定在该花色——只能留该花色数牌 + 字牌（東南西北中發白），其余花色一律尽快打掉，绝不留其他花色在手里。注意：引擎算出的 candidates / legal 有时仍含其他花色（引擎不感知锁门），你必须无视它们，只从「锁定花色 + 字牌」里做取舍。',
